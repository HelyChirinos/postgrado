...
@section('content')
    <div class="card mb-2">
        <div class="card-header d-print-none">
            <div class="row">
                <div class="col-11 text-center fs-5">Recibos: del {{formatFecha($desde)}} al {{formatFecha($hasta)}} </div>
                ...
                <table id="sqltable" class="table table-bordered table-striped table-sm table-hover dataTable" style="width: 80%">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{$item->concepto}}</td>
                            <td>{{$item->no_depo}}</td>
                            <td>{{formatFecha($item->fecha_depo)}}</td>
                            <td>{{formatMoney($item->monto_depo)}}</td>
                            <td>{{$item->status}}</td>
                        </tr>
                        @empty
                            <tr>
...
<script type="module">
        ...
        let pdfButton = {
            ...
            action: function(e, dt, node, config) {
                dt.state.clear();
                let url='{{ route('back.pdf.recibos.periodo')}}';
